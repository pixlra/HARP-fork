...
public:
  ...
  CShow_PredictionUnits(TComPic* pcPic)
  {
    ...
    else //full image
    {
      ...
      CTU_Span = CTU_LowerRight - CTU_UpperLeft;
      ImgDim.x = (CTU_Span.x)*64*Zoom; //this is equal to 800x480 * Zoom
      ImgDim.y = (CTU_Span.y)*64*Zoom;
    }
...
//		  MV_y.append((*it3).y);
    ...
    // EXPORTING IMAGES
    //---------------------------------------------
    Global.exportImage(Blended,  "PredictionUnits", true);
	Global.exportImage(Blended,  "CurPOC_PredictionUnits", false);
    Global.exportImage(Resized1, "Orig", true);
	Global.exportImage(Resized1, "CurPOC_Orig", false);

    //---------------------------------------------
    // EXPORTING PKL
    //---------------------------------------------
    //save_CU(PyPOC, Global.TmpDir + "PyPOC.pkl");
...
//
//    assertParameters(rpcPic, iCUAddr);
//
//    //unused
...
//    Point CU_Anc  = Point(AbsPart.x, AbsPart.y ) * 4 * Zoom; //times 4 since one Smallest-Partition spans 4x4 pixels!
//    Point CU_Dims = Point(CU_Width , CU_Width ) * Zoom;
//    //Mat CU_Roi = CTU_Roi(Rect(CU_Anc.x, CU_Anc.y, CU_Dims.x, CU_Dims.y));
...
//    Point StorageUnits = Point(CU.Width/4, CU.Width/4);
//    assert(StorageUnits.x * StorageUnits.y == NumPartInCU);
//
//    if(isDrawingStoreUnits)
//      for(int y = 0; y<StorageUnits.y; y++)
//        for(int x = 0; x<StorageUnits.x; x++)
//        {
//          Point SU_Anc  = Point(x*4, y*4 ) * Zoom; //times 4 since one StorageUnit spans 4x4 pixels!
//          Point SU_Dims = Point(4 , 4 ) * Zoom;
//          Mat SU_Roi = CU_Roi(Rect(SU_Anc.x, SU_Anc.y, SU_Dims.x, SU_Dims.y));
//
...
//        int CTUAddr = pcCU->getAddr();
//        char CUName[500];
//        sprintf(CUName, "ZorderIdx%04d", uiAbsZorderIdx);
//        PyPOC["CTUs"][CTUAddr][CUName] = Tab();
//        Tab& PyCU = PyPOC["CTUs"][CTUAddr][CUName];
//        PyCU["PU Partitions"] = PartitionSizeStrings[pcCU->getPartitionSize(uiAbsZorderIdx)];
//        PyCU["CU Depth"] = pcCU->getDepth(uiAbsZorderIdx);
...
//	Info["IntraPercentage"] = 0;
//	Info["InterPercentage"] = 0;
//	Info["MergePercentage"] = 0;
//	Info["SkipPercentage"] = 0;

    //float NumPixels =  pcPicYuv->getWidth() * pcPicYuv->getHeight();  //does not account for possible borders
    //float NumPixels = uiWidthInLCUs * uiHeightInLCUs * 64 * 64;
