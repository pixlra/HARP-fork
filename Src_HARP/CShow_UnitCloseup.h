...
public:
  ...
  CShow_UnitCloseup(TComDataCU* pcCU)
  {
    ...
    //---------------------------------------------
    // PREPARING CTB-IMAGES AND BACKGROUND IMAGE
    //---------------------------------------------
    ImgDimX = 64*Zoom;
    ImgDimY = 64*Zoom;
    ...
    // EXPORTING IMAGES
    //---------------------------------------------
    Global.exportImage(Closeup,  "UnitCloseup", true, true);
	Global.exportImage(Closeup,  "CurPOC_TransformUnits", false);
...
//      return;
    ...
    //HACK: setting LCU_x and LCU_y to zero since we only want to draw current CTB only
    LCU_x = 0;
    LCU_y = 0;
	...
	if(pcCU->getPredictionMode(uiAbsZorderIdx) == MODE_INTRA)
	  ...
	  color = GRAY; //used for outer bounds CB's
